...
| `pnpm reset:seed`                     | Repopulate `.skills-bank.json` source markers in this repo's `skills/`. Walks via `walkSkills`, so naturally operates on whichever buckets exist (post-v1.1, only `skills/vendored/`). Auto-invoked by `reset:hard`; runnable standalone after a fresh checkout.                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `pnpm update:skill <name>`            | Pull a locally-edited skill from `~/.claude/skills/<name>/` (override with `--from`) back into this repo under `skills/personal/<name>/` or `skills/vendored/<name>/`. Bucket is auto-detected when the skill already exists; pass `--bucket personal\|vendored` for new skills. Schema-validates `meta.json`. Preserves the destination's `.skills-bank.json` / `.skills-bank-hash` provenance markers across the content refresh. Wipes other destination files so removed-upstream files disappear from the bank too. `--dry` previews without writing. Does NOT create commits — leaves the working tree for `pnpm validate && pnpm build:index` + `git diff` review. |
| `pnpm migrate:source-markers`         | One-shot eager migration of committed `.skills-bank.json` markers to the post-v1.3 vocabulary (`bundled`→`curated`, `yours`→`user`, JSON field `upstream`→`origin`). `--dry` previews without writing; `--root <path>` targets a different registry. The app's read path tolerates both old and new forms for one minor cycle, so this is optional cleanup — but running it once contains the migration diff to a single PR rather than dripping through over weeks.                                                                                                                                                                                                      |
...
- **Source axis values are `curated` / `user`** (legacy `bundled` / `yours` still tolerated on read in `packages/core/src/source.ts`; writes always emit the new form). The `.skills-bank.json` field `origin` replaces `upstream` under the same tolerant-read window. Every install starts on the curated default; GitHub linking moves to Settings → Account.
- **Registry manifest is v3** (see `packages/core/src/manifest.ts`). Each `ManifestSkill` carries an explicit `bucket: "personal" | "vendored"` decoupled from the source axis. Legacy v2 manifests coerce up through `coerceManifestToCurrent` — single quarantined chokepoint, no version branches downstream. v1 manifests are no longer readable.
