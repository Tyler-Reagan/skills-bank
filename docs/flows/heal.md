...
| **edited-without-origin**   | Local copy of a curated skill (no `origin` pointer) differs from the synced baseline.     | Two arms: **Keep my edits** detaches from Pull (skill becomes `source: user`, future pulls stop overwriting); **Re-baseline** snapshots the current state as the new synced version (drift clears, Pull still owns the skill).                              |
| **edited-with-origin**      | Local copy of a skill with a GitHub `origin` pointer has diverged from that upstream.     | Two arms: **Keep my edits** unlinks the origin pointer; **Reset to origin** re-fetches the upstream content. Both clear the `DRIFT` badge.                                                                                                                 |
...
- **Single reasonable option**: the dialog renders one heal button with explanatory copy beneath it (no modal). Today this covers edited-without-origin, registry-folder-missing, and external-target-missing.
...
### Edited-without-origin — surfaced through the dialog

The dialog for a curated skill you've edited replaces the usual destructive area with two heal buttons — **Keep my edits** and **Revert to canon** — under explanatory copy. The card carries the `DRIFT` badge. Both buttons clear the badge; the difference is _which copy survives_: Keep my edits detaches the skill from Pull (the skill becomes `source: user`), while Revert to canon re-baselines the synced hash so Pull still owns the skill and the next pull may overwrite local edits.

_Screenshot deferred — capture by editing a curated skill's `SKILL.md` in the app's data dir (`~/Library/Application Support/@skills-bank/desktop/registry/skills/<name>/`) and refreshing the Registry tab._
