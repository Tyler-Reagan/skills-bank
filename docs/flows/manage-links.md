...
An install collision means there's already a real folder or foreign symlink at that path. Skills Bank refuses to silently overwrite it — that's how you avoid losing work that another tool placed there. Resolve the collision first ([heal.md](heal.md)) and the row becomes editable.
