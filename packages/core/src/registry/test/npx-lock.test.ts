...
describe("adoptableNpxSkills", () => {
  test("returns npx skills whose name is not already in the registry", () => {
    const result = adoptableNpxSkills(FIXTURE, ["code-review"]);
  ...
  test("excludes every npx skill when all are already registered", () => {
    const result = adoptableNpxSkills(FIXTURE, [
      "code-review",
      "gitlab-skill",
      "local-only",
    ]);
  ...
  test("maps a GitHub source to a full origin row", () => {
    const result = adoptableNpxSkills(FIXTURE, ["gitlab-skill", "local-only"]);
    expect(result).toEqual([
      {
        name: "code-review",
        origin: {
  ...
  test("adopts a non-GitHub (GitLab) source rather than filtering it out", () => {
    const result = adoptableNpxSkills(FIXTURE, ["code-review", "local-only"]);
    expect(result).toEqual([
      {
        name: "gitlab-skill",
        origin: {
  ...
  test("a local entry is still adoptable but carries a null origin", () => {
    const result = adoptableNpxSkills(FIXTURE, ["code-review", "gitlab-skill"]);
    expect(result).toEqual([{ name: "local-only", origin: null }]);
  });

  test("empty lockfile yields nothing", () => {
    expect(adoptableNpxSkills({}, ["code-review"])).toEqual([]);
  });
  ...
  test("result is sorted by name for stable rendering", () => {
    ...
    expect(adoptableNpxSkills(unordered, []).map((s) => s.name)).toEqual([
      "alpha",
      "zebra",
    ]);
  });
