...
| AppUpdateNotesModal.tsx                 | 153  | App release notes + download/restart flow (app updates — distinct from skill SkillUpdatesModal)                                                                                                                                                                                                                                                                                             | ModalHost                                                                        |
| browseFilters.ts                        | 158  | Browse-tab filter/sort algebra: `RegistryFilterTag` / `RegistrySortState` types, `CHIP_DEFS` (chip predicate + label + title), `applyChipFilters` (AND), `applyFilters` (search/tag/installed-only), `applySort`, `floatToTop`. Consumed by BrowseTab + the browse hooks, so kept out of `RegistryFilters.tsx`                                                                              | BrowseTab, RegistryFilters, useBrowseFilters, useOriginProbe                     |
| BrowseTab.tsx                           | 544  | Registry view; collapsible category sections, `GhostBand` manifest-import ghost cards, search/tag/sort filter pipeline                                                                                                                                                                                                                                                                      | App                                                                              |
...
| ConflictResolver.tsx                    | 288  | Generic resolver skeleton: Modal + bulk "Select all" toolbar + per-row `ConflictActionPicker` + live tally + Cancel/Apply footer. Picks are controlled — state lives in each domain wrapper                                                                                                                                                                                                 | InstallCollisionModal, ManifestConflictModal; ManageLabelsModal (picker only)    |
...
| Header.tsx                              | 394  | App chrome: brand, "Check for skill updates" + "Check for app updates" buttons, theme + density toggles, account chip; `ImportIndicator` manifest-progress chip internal                                                                                                                                                                                                                    | App, SettingsContext                                                             |
...
| installedGrouping.ts                    | 147  | Pure logic for the Installed tab: `InstalledGroup`/`ClassifiedGroup` types, `aggregateByName` (dedupe per-agent installs by name, upgrade-to-`ours`), `selectResolvableConflicts` (registered vs unregistered conflict-picker rule), `syntheticEntryFromInstall` (stand-in registry entry), private `kindRank`. Kept out of the `.tsx` so App/ModalHost import from non-presentational code | InstalledTab, App, ModalHost                                                     |
| InstalledTab.tsx                        | 330  | Installed view orchestrator: computes groups/classification, renders intro + counts + a "Recheck" refresh action, composes `NeedsAttentionSection` and the inline unregistered / registered sections                                                                                                                                                                                        | App, ModalHost                                                                   |
...
| ModalHost.tsx                           | 883  | Modal router: `ActiveModal` discriminated union → one rendered modal, plus inline handlers for overwrite and bulk-repair (resolve-all now its own component, `ResolveAllConflictsModal.tsx`); internal `DrawerHost` wires SkillDetailDrawer (capability-gated callbacks)                                                                                                                    | App                                                                              |
...
| NeedsAttentionSection.tsx               | 175  | Installed tab's "Needs attention" section: bulk repair/resolve header buttons + per-card inline action routing (repair-broken / resolve-conflicts / resolve-registration-conflicts / forget-missing). Extracted from an inline IIFE in InstalledTab                                                                                                                                         | InstalledTab                                                                     |
| primitives.tsx                          | 191  | Stateless micro-primitives: `SearchBar`, `InfoTooltip`, `DisclosureChevron`, `SkillTagList`, `SplashScreen`                                                                                                                                                                                                                                                                                 | App, BrowseTab, BulkInstallModal, InstalledTab, ManageLabelsModal, RepoTransport |
...
| ResolveAllConflictsModal.tsx            | 134  | Bulk "Resolve all conflicts" confirm + sweep from the Installed tab's Needs-attention bulk action: replace-with-symlink across every target skill, per-skill error tally, retry-in-place on partial failure. Extracted from an inline ModalHost render (real multi-step state — `running`/`errors`)                                                                                         | ModalHost                                                                        |
...
| SkillUpdatesModal.tsx                   | 162  | Skill-origin updates list with per-row and update-all actions (skill updates — distinct from app-release AppUpdateNotesModal)                                                                                                                                                                                                                                                               | ModalHost                                                                        |
...
| App.tsx                  | 842  | Root composition: provider nest (ModalRegistry → RegistryHost → Settings → Registry → Labels), tab state, suggested-action dispatch, review-session state, ModalHost wiring |
...
`modal-body--w${width}` (modalStyles), `bulk-install-progress-row--${status}`
(BulkInstallModal), and `origin-probe-${originProbeState.phase}` (Header). A
selector whose every compound requires a dead class can never match and is
safe to delete.

Last run 2026-07-09 (renderer-altitude + vocabulary pass, two passes):
first removed the orphaned `labels-banner*` section (4 classes —
`LabelsFirstRunBanner` was dead code, never rendered) and renamed
`.rescan-*` → `.origin-probe-*` for the Header update-check button's
three instance-specific classes, leaving `.rescan-view-cta` shared with
the Local Scan button's own "· Review" CTA. Then, once Local Scan was
retired outright, removed its four `.local-scan-category*`/`.local-scan-
item*` classes and renamed the now-single-consumer `.rescan-view-cta` →
`.origin-probe-view-cta`.

Prior run 2026-06-09 (v1.21 consolidation): removed the orphaned
...
| File                         | LOC | Purpose                                                                                 |
| ---------------------------- | --- | --------------------------------------------------------------------------------------- |
| useAppUpdateFeed.ts          | 65  | Auto-update status feed + per-version dismissal gate                                    |
...
| useOriginProbe.ts            | 177 | Header "Check for skill updates" button state machine end-to-end                        |
...
- **App.tsx** (842L): largest renderer file; prior extractions (contexts,
...
- **ModalHost** (883L incl. internal DrawerHost): resolver routing is thin
  pass-throughs; resolve-all became its own component
  (`ResolveAllConflictsModal.tsx`) once its `running`/`errors` state made
  it a real multi-step flow — the trigger this observation predicted. The
  remaining inline overwrite / bulk-repair handlers stay thin
  pass-throughs; extract either if it grows the same way.
