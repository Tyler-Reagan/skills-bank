...
| InstalledTab.tsx                        | 443  | Installed view orchestrator: computes groups/classification, renders intro + counts, composes `NeedsAttentionSection` and the inline unregistered / registered sections; `LocalScanResultsSection` sub-component still inline                                                                                                                                                               | App, ModalHost                                                                   |
...
| NeedsAttentionSection.tsx               | 158  | Installed tab's "Needs attention" section: bulk repair/resolve header buttons + per-card inline action routing (repair-broken / resolve-conflicts / resolve-registration-conflicts). Extracted from an inline IIFE in InstalledTab                                                                                                                                                          | InstalledTab                                                                     |
...
| App.tsx                  | 951  | Root composition: provider nest (ModalRegistry → RegistryHost → Settings → Registry → Labels), tab state, suggested-action dispatch, review-session state, ModalHost wiring |
...
(BulkInstallModal), `origin-probe-${originProbeState.phase}` and
`local-scan-${localScanState.phase}` (Header). A selector whose every
compound requires a dead class can never match and is safe to delete.

Last run 2026-07-09 (renderer-altitude + vocabulary pass): removed the
orphaned `labels-banner*` section (4 classes — `LabelsFirstRunBanner` was
dead code, never rendered) and renamed `.rescan-*` → `.origin-probe-*`
for the three instance-specific classes on the Header's update-check
button (`.rescan-view-cta` stays shared with the Local Scan button's own
"· Review" CTA until that button is retired).
