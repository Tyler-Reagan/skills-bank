...
2. The modal loads a diff preview against what's currently in your repo's `registry-manifest.json`:
   ...
   - **Changed** — skills present in both but with differing origin or labels (category / tags).
...
4. Click **Commit directly** or **Export as PR**.
...
3. If the file doesn't exist yet you'll see a "No manifest in repo yet — export one first" message.
4. Click **Import** to begin. Each skill is re-fetched from its origin; a progress indicator updates per-skill.
